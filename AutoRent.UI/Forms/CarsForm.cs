...
namespace AutoRent.UI.Forms
{
    public partial class CarsForm : Form
    {
        ...
        private async void DataGridViewCars_CellDoubleClick(object? sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataGridViewCars.Rows[e.RowIndex].DataBoundItem is Car car)
            {
                await EditCarAsync(car);
            }
        }

        private async Task EditCarAsync(Car car)
        {
            using var dlg = new CarEditForm(_context, car);
            if (dlg.ShowDialog(this) == DialogResult.OK)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Error("CarsForm.EditCarAsync: " + ex);
                }
            }
        }
